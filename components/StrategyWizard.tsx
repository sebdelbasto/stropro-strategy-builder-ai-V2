...
const DEFAULT_ARTICLE =
  process.env.NEXT_PUBLIC_ARTICLE_BASE ||
  'https://www.stropro.com/investments-solutions'
...
export default function StrategyWizard({ embedMode }: { embedMode?: boolean }) {
  ...
  function pushUser(msg: string) { transcriptRef.current.push({ role: 'user', content: msg }) }
  function pushAI(msg: string) { transcriptRef.current.push({ role: 'assistant', content: msg }) }

  async function generateIdeas() {
    setLoading(true)
    pushUser(`Generate ${inputs.objective} ideas for ${inputs.underliers.join(', ')} over ${inputs.tenorMonths}m (${inputs.riskProfile}, ${inputs.investmentCurrency}). Notes: ${inputs.notes || '—'}`)
    try {
        ...
        const list: Suggestion[] = [
          {
            ...
            explainer: `Illustrative ${inputs.objective.toLowerCase()} structure over ${years} years on ${inputs.underliers.join(', ')}. Tuned to ${inputs.riskProfile} in ${inputs.investmentCurrency}.`,
          ...
          },
          {
            ...
            explainer: `Variant trading income vs. protection; aligned to ${inputs.riskProfile}.`,
            articleUrl: DEFAULT_ARTICLE,
      ...
      <div className="flex items-center gap-3">
        {['Objective', 'Parameters', 'Ideas', 'Submit'].map((label, i) => (
          <div key={i} className="flex items-center gap-2">
            <span className={`badge ${step === i ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : ''}`}>{i + 1}</span>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {OBJECTIVES.map((obj) => (
                <button
                  ...
                  className={`btn-secondary text-left ${inputs.objective === obj ? 'ring-2 ring-stroBlue' : ''}`}
                >
                  {obj}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setInputs((v) => ({ ...v, investmentCurrency: e.target.value as any }))}
              >
                <option>USD</option>
                <option>AUD</option>
                <option>EUR</option>
            ...
            <div>
              <label className="label">Underliers (comma separated – e.g., SPX, SX5E, AAPL)</label>
              <input
                className="input"
            ...
            <label className="inline-flex items-center gap-2 md:col-span-2">
              <input
                ...
                onChange={(e) => setInputs((v) => ({ ...v, investorTypeConfirmed: e.target.checked }))}
              />
          ...
          <div className="card space-y-3">
            ...
            <div className="grid gap-3">
              {suggestions.map((s, idx) => {
                const link = s.articleUrl || DEFAULT_ARTICLE
                return (
                  <div
                    key={`${s.title}-${idx}`}
                    className={`rounded-2xl p-4 border ${selected === s ? 'border-stroBlue' : 'border-zinc-200 dark:border-zinc-800'}`}
                  >
                      ...
                      <div>
                        ...
                        {s.indicativeCoupon && (
                          <div className="small mt-1">
                            Indicative: {s.indicativeCoupon} <span className="opacity-60">(illustrative only)</span>
                          </div>
                        )}
                      </div>
                      <button type="button" className="btn-secondary" onClick={() => setSelected(s)}>
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="button" className="btn-primary disabled:opacity-40" disabled={!selected} onClick={() => setStep(3)}>
                Price this strategy
              </button>
            </div>
          </div>
...
function Disclosure() {
  return (
    <div className="card small leading-relaxed">
      <strong>Important:</strong> Educational prototype for licensed advisers/wholesale clients only. Not personal advice.
      Indicative ranges are illustrative and depend on live markets & final terms.
    </div>
  )
}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className="input" value={form.firm} onChange={(e) => setForm((v) => ({ ...v, firm: e.target.value }))} />
            </div>
            <div>
