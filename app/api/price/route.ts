...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sgKey)
      return NextResponse.json({ ok: false, message: 'SENDGRID_API_KEY missing', echo: data }, { status: 200 });

    sgMail.setApiKey(sgKey);

    const subject = `New Pricing Request – ${data.inputs.objective} – ${data.inputs.underliers?.join(', ')}`;
    const html = `
      <h2>${escapeHtml(process.env.COMPANY_NAME || 'Company')} – Strategy Pricing Request</h2>
      <p>
        <strong>Name:</strong> ${escapeHtml(data.contact.name)}<br/>
        <strong>Email:</strong> ${escapeHtml(data.contact.email)}<br/>
        <strong>Firm:</strong> ${escapeHtml(data.contact.firm || '')}<br/>
        <strong>Phone:</strong> ${escapeHtml(data.contact.phone || '')}
      </p>
      <hr/>
      <h3>Inputs</h3>
