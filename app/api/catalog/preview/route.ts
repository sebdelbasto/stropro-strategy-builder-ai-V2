...
export async function POST(req:Request){
  ...
  const { objective='Enhanced Income', underliers=['SPX'], tenorMonths=24, investmentCurrency='AUD' } = inputs
  try {
    ...
    return NextResponse.json({
      ...
      samplePeek: cohort.slice(0,8).map(p => ({
        id: p.id, name: p.name, type: p.type, currency: p.currency, tenorMonths: p.tenorMonths,
        underliers: p.underliers, couponPct: p.couponPct
      }))
    })
  } catch (e:any) {
